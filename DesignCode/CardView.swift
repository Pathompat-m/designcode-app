            ...
            TimelineView(.animation) { context in
                card.image
                    ...
                    .if(hasNoise, transform: { view in
                        view.overlay(RoundedRectangle(cornerRadius: 20)
                    ...
                    .if(hasEmboss, transform: { view in
                        view.layerEffect(ShaderLibrary.emboss(.float(1)), maxSampleOffset: .zero, isEnabled: hasEmboss)
                    })
                    ...
                    .onReceive(numberTimer, perform: { _ in
                        guard isPixellated || hasEmboss else {
                            return }
                        if isIncrementing {
                            number += 1
                        } else {
